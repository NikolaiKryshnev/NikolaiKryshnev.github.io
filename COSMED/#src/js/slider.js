...
for (let i = 0; i < $('.slider--js').length; i++) {
	...
	if ($(window).width() < 1240 ) {
		$($slider).parents('._section').find('.section-title').append(`<div class="blocks-arrow"></div>`)
		
		$($slider).parents('._section').find('.blocks-arrow').append( $($slider).find('.slick-arrow'))
