...
// logoAppend
function logoAppend() {
	let headerLog = document.querySelector('.header__logo'),
		menu = document.querySelector('.menu');
	menu.appendChild(headerLog)
	menu.insertBefore(headerLog, menu.firstChild)
}
...
if (window.innerWidth <= 991) {
	function logogMobile() {
		let headerLogoMobile = document.querySelector('.header__logo').querySelector('img'),
		headerLogoAttr = headerLogoMobile.getAttribute('data-mobileimg');
		headerLogoMobile.setAttribute('src', `${headerLogoAttr}`)
...
function cloneCheckoutResult(parent, child) {
	if (parent != null) {
		...
		if (child.classList.contains('header')) {
			imgAttr = clone.querySelector('img').getAttribute('data-mobileimg');
			if (imgAttr != null) {
				clone.querySelector('img').setAttribute('src', `${imgAttr}`)
			}
		}
	}
}

cloneCheckoutResult(document.querySelector('.sidebar__checkout'), document.querySelector('.shop_table'));
cloneCheckoutResult(document.querySelector('body'), document.querySelector('header.header'));
...
for (let index = 0; index < header.length; index++) {
	...
	let iconMenu = el.querySelector(".icon-menu");
	if (iconMenu != null) {
		let delay = 500;
		let menuBody = el.querySelector(".menu__body");
...
function scroll_scroll() {
	...
	let src_value = pageYOffset,
		...
		headerClone = document.querySelector('header._clone'),
		sideBlock = document.querySelector('._side-block__wrapper');

	if (header !== null) {
		if (src_value > (header.offsetHeight + 80)) {
			headerClone.classList.add('_scroll');
			header.querySelector('.menu-body').classList.remove('_active');
			header.querySelector('.icon-menu').classList.remove('_active');
			if (sideBlock !== null) {
				sideBlock.style.paddingTop = `${headerClone.offsetHeight / 2}px`;
			}
		} else {
			headerClone.classList.remove('_scroll');
			headerClone.querySelector('.menu-body').classList.remove('_active');
			headerClone.querySelector('.icon-menu').classList.remove('_active');
			if (sideBlock !== null) {
				sideBlock.style.paddingTop = `0`;
			}
		}
	}
...
function fix_block(scr_fix_block, scr_value) {
	...
	let header_height = parseInt(document.querySelector('header').offsetHeight);
	for (let index = 0; index < scr_fix_block.length; index++) {
		...
		if (window_width > block_width) {
			...
			if (scr_value > (header_height)) {
				item.style.cssText = "position:fixed;  margin-top: 0;bottom:auto;top:" + 0 + "px;width:" + block.offsetWidth + "px;left:" + offset(block).left + "px; ";
			} else {
				gotoRelative(item);
			}
			...
			if (scr_value > (block.offsetHeight + offset(block).top) - (item.offsetHeight)) {
				...
				console.log(scr_value, (block.offsetHeight + offset(block).top) - (item.offsetHeight));
